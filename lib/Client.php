...
class Client
{
    ...
    public function create_and_sign_transaction($data, $keys = [])
    {
        ...
        if (
            isset($data['data']['expected_unsigned_txid'])
            && !is_null($data['data']['expected_unsigned_txid'])
            && $data['data']['expected_unsigned_txid'] != $unsigned->get()->getTxId()->getHex()
        ) {
        ...
        if (!is_null($this->pin)) {
            ...
            if (isset($data['data']['user_key']) && !array_key_exists($data['data']['user_key']['public_key'], $this->userKeys)) {
    ...
    { // extracts user key by using the appropriate algorithm
        ...
        $encryption_key = $this->pinToAesKey(
            $pin,
